...
export class ChatController {
  ...
  async addPassword(
    ...
    await this.chatService.addPassword(
      request.user,
      body.chatId,
      body.password,
    );
  ...
  async changeRoomOwner(
    ...
    await this.chatService.changeRoomOwner(
      request.user,
      body.chatId,
      body.userId,
    );
  ...
  async inviteUser(
    ...
    await this.chatService.inviteUserToChat(
      request.user,
      body.chatId,
      body.userId,
    );
  ...
  async uninviteUser(
    ...
    await this.chatService.removeInviteToChat(
      request.user,
      body.chatId,
      body.userId,
    );
  ...
  @Get('/user/invite')
  async getUserInvites(@Req() request: RequestWithUser): Promise<Chat[]> {
  ...
  @Get('/uninvite')
  async getChatUninvites(@Req() request: RequestWithUser): Promise<Chat[]> {
  ...
  async getDirectMessages(@Req() request: RequestWithUser) {
    ...
    const result = await Promise.all(
      messages.map(async (dm) => {
        return !(await this.friendService.haveBlockRelation(
          dm.userOne,
          dm.userTwo,
        ));
      }),
  ...
  async authorizeId(
    ...
    const result = await this.chatService.authorizeDirectMessage(
      request.user,
      id,
    );
    if (!result) {
