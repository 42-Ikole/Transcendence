...
import { RequestWithUser, TwoFactorCodeDto, UserWithTwoFactor } from 'src/auth/auth.types';
...
export class TwoFactorController {
	...
	private async validateCode(user: UserWithTwoFactor, code: string) {
		const isCodeValid = await this.twoFactorService.isTwoFactorCodeValid(code, user);
		if (!isCodeValid) {
	...
	async generateQrCode(@Req() request: RequestWithUser, @Res() response: Response) {
		const { otpauthUrl } = await this.twoFactorService.generateTFASecret(request.user);
		return this.twoFactorService.pipeQrCodeStream(response, otpauthUrl);
	}

	@Post('enable')
	@UseGuards(AuthenticatedGuard)
	async enableTwoFactor(@Req() request: RequestWithUser, @Body() { twoFactorCode }: TwoFactorCodeDto) {
	...
	async authenticateTwoFactor(@Req() request: RequestWithUser, @Body() { twoFactorCode }: TwoFactorCodeDto) {
