...
export default defineComponent({
  ...
  methods: {
    async inviteUser(user: PublicUser) {
      await makeApiCallJson("/chat/invite", "POST", {
        chatId: this.chatId,
        userId: user.id,
      });
    },
    async uninviteUser(user: PublicUser) {
      await makeApiCallJson("/chat/invite", "DELETE", {
        chatId: this.chatId,
        userId: user.id,
      });
    },
    async refreshInvites() {
      ...
      if (inviteResponse.ok) {
        ...
        this.invitedUsers = new Set(
          this.invitedUsersArray.map((user) => user.id)
        );
      }
    },
  },
